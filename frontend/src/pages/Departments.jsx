...
export default function Departments() {
  ...
  return <div><PageHeader backTo="/" title="Departments" subtitle="Organize people, assets and ownership by team." actions={canEdit?<button onClick={()=>setShowForm(v=>!v)} className="pill-accent flex items-center gap-1.5 px-4 py-2.5 text-sm">{showForm?<X size={15}/>:<Plus size={15}/>} {showForm?"Cancel":"Add Department"}</button>:null}/>
  {showForm&&canEdit&&<form onSubmit={e=>{e.preventDefault();if(name.trim())create.mutate()}} className="card mb-5 grid gap-3 p-5 sm:grid-cols-2"><TextField label="Department name" value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Engineering" required/><label><span className="text-xs font-medium text-muted">Department manager</span><select className="field mt-1 w-full" value={managerId} onChange={e=>setManagerId(e.target.value)}><option value="">No manager assigned</option>{employees.filter(e=>e.status!=="LEFT_COMPANY").map(e=><option key={e.id} value={e.id}>{e.name} · {e.role}</option>)}</select></label><div className="sm:col-span-2 flex justify-end"><button type="submit" disabled={create.isPending} className="pill-accent px-5 py-2.5 text-sm">{create.isPending?"Creating…":"Create"}</button></div>{error&&<p className="sm:col-span-2 text-sm text-danger">{error}</p>}</form>}
  {isLoading?<p className="text-sm text-muted">Loading…</p>:<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{departments.map((dept,i)=><div key={dept.id} className="card p-5"><div className="flex items-start justify-between"><IconChip icon={Building2} tone={TONES[i%TONES.length]} size="md"/>{canEdit&&<button onClick={()=>{if(window.confirm(`Delete ${dept.name}?`))remove.mutate(dept.id)}} className="rounded-full p-2 text-muted hover:bg-red-50 hover:text-danger"><X size={14}/></button>}</div><p className="mt-4 text-lg font-semibold text-ink">{dept.name}</p><div className="mt-4 flex items-center gap-4 text-xs text-muted"><span className="inline-flex items-center gap-1.5"><Users size={12}/> {dept._count?.employees||0} employees</span><span className="inline-flex items-center gap-1.5"><Boxes size={12}/> {dept._count?.assets||0} assets</span></div><div className="mt-4 border-t border-border pt-4"><div className="flex items-center gap-2"><UserRound size={14} className="text-muted"/><span className="text-[10px] font-semibold uppercase tracking-wide text-muted">Manager</span></div>{canEdit?<select value={dept.manager?.id||""} onChange={e=>assignManager.mutate({id:dept.id,managerId:e.target.value})} className="field mt-2 w-full py-2 text-xs"><option value="">No manager</option>{employees.filter(e=>e.status!=="LEFT_COMPANY").map(e=><option key={e.id} value={e.id}>{e.name}</option>)}</select>:<p className="mt-2 text-xs text-muted">{dept.manager?.name||"No manager"}</p>}</div></div>)}{departments.length===0&&<div className="sm:col-span-2 lg:col-span-3"><EmptyState icon={Building2} title="No departments yet" description="Create departments to organize employees and assets."/></div>}</div>}
  </div>
}
